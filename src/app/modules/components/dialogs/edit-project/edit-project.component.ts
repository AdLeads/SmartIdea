...
export class EditProjectComponent implements OnInit {
  ...
  loadSkillList(): Skill[] {
    console.log(this.data.idproject);
    this.serviceSkill
      .getSkillsproject(this.data.idproject)
      .subscribe((data) => {
        this.skills = data;
      });
    return this.skills;
  ...
  areaDelet(idarea: number, name: string) {
    ...
    this.areaService.editarea(idarea, area).subscribe((area) => {
      console.log(area);
    });
    this.onNoClick();
  }
  ...
  add(event: MatChipInputEvent): void {
    ...
    if ((value || '').trim() && !this.taagrept(tag)) {
      this.tags.push(tag);
      this.tagSerive
        .posttagproject(this.data.idproject, tag)
        .subscribe((tag) => {
          console.log(tag);
        });
    }
  ...
  remove(tag: Tag): void {
    ...
    if (index >= 0) {
      ...
      this.tagSerive
        .deleteprojecttotag(tag.tagId, this.data.idproject)
  ...
  selected(event: MatAutocompleteSelectedEvent): void {
    ...
    if (!this.taagrept(tag)) {
      this.tags.push(tag);
      this.tagSerive
        .addprojecttotag(this.data.idproject, tag)
